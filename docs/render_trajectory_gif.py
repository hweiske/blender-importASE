...
mod[idents['pair_table']].data.update()
mod[idents['element_table']].data.update()
struct.update_tag()

frames = bpy.data.scenes['Scene'].frame_end
bpy.context.scene.frame_set(1)
deps = bpy.context.evaluated_depsgraph_get()
ev = struct.evaluated_get(deps); me = ev.to_mesh()
co = np.array([v.co[:] for v in me.vertices])
co = co @ np.array(struct.matrix_world.to_3x3()).T + np.array(struct.matrix_world.translation)
lo, hi = co.min(0), co.max(0); center = (lo + hi) / 2
ev.to_mesh_clear()

import math
from mathutils import Euler
cam = bpy.data.objects['Camera']; cam.data.type = 'ORTHO'
cam.data.ortho_scale = 22.0
cam.data.clip_start = 0.01; cam.data.clip_end = 100000
