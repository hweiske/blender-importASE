...
- **Package:** `blender_importASE/` (add-on version 2.6.0, min Blender 4.4; tested on 4.4, 5.1 and 5.2).
...
import_ase_molecule(filepath, filename, overwrite=True, add_supercell=True,
    ...
    outline=True, adps=True, adp_probability=0.5, hydrogen_adps=False, **kwargs)
```
Reads via `ase.io.read(index=':')` (VASP CHGCAR-family via `read_vasp_density`, SHELX `.res` / `.ins` via `shelx.read_shelx`; CIFs keep their atom-site tags), builds a collection named `<formula>_<stem>`, dispatches by `representation`, optionally draws the unit cell and reads the density volume. With the nodes representation, a static structure whose file has anisotropic displacement parameters gets thermal ellipsoids by default (`adps`; see [Polyhedra](#polyhedra--polyhedraimport_polyhedra) for how they are read and drawn).
...
import_polyhedra(filepath, filename, expand_cutoff=1.2, trim_cutoff=1.0,
    ...
    cell_margin=0.0, all_images=True, framework_shells=1, unit_cell=False,
    adps=True, adp_probability=0.5, hydrogen_adps=False, **kwargs)
```
Builds a coordination polyhedron (convex hull, `scipy.spatial.ConvexHull`) around every atom with ≥`min_neighbors` neighbors within `poly_cutoff`×covalent radius. Produces the atoms/bonds structure object **plus** a separate `<name>_faces` mesh carrying `atom_color`/`element` attributes and the `'polyhedra material'` — a Principled BSDF (roughness 0.6, IOR 1.45, alpha 0.3) and a Glass BSDF (multiscatter GGX, roughness 0, IOR 1.5) mixed half and half, both tinted by `atom_color`: the principled half carries the color and the transparency, the glass half the refraction and the bright edges. It is only built when the material does not already have that glass half, so an older one is rebuilt and tweaks to a current one survive a re-import. Outline (when on) goes on the atoms/bonds only, never the faces. `unit_cell=True` draws the cell (skipped when the file has no 3d cell). The structure object carries the same modifier stack as the nodes representation — `hide atoms` (per-element `cutoff_<El>` toggles), `supercell` (periodic input only; built from the file's cell), atoms_and_bonds, outline. The faces object gets a `supercell` modifier with the same node group, every input driven by the structure's, so the polyhedra repeat along with the atoms; its Merge by Distance also collapses polyhedra that coincide across the cell boundary.

**SHELX files.** `.res` / `.ins` files are read by `shelx.read_shelx` (ASE has no reader for them) — in this importer and in `import_ase_molecule`, both through `adp.read_atoms` — with or without ADPs: CELL, LATT (inversion from its sign, centering from its magnitude), SYMM, SFAC (short and long form), FVAR and the atom lines — continuation lines, fixed (`10+p`) and free-variable (`10m+p`, `-(10m+p)`) parameters decoded as SHELXL does, riding hydrogens (`Uiso` between −5 and −0.5) as that multiple of the previous non-hydrogen atom's U_eq. Everything after END (the Q peaks of a `.res`) is skipped. The sites are expanded through `ase.spacegroup.crystal` like a CIF and stored with CIF-style atom-site tags, so the ADP code below is shared.

**Thermal ellipsoids** (`adps=True`, the "ADPs" checkbox, **on by default** here and in `import_ase_molecule` with the nodes representation). They are drawn only when the file carries an anisotropic displacement table (`adp.anisotropic_adps`: a CIF `_atom_site_aniso_*` loop or anisotropic SHELX atoms); any other file — no CIF/SHELX, or isotropic values only — imports exactly as without the option. Trajectories and the non-nodes representations are left alone. `adp.py` reads the displacement parameters ASE keeps with `read(..., store_tags=True)`: `_atom_site_aniso_U_ij`, `B_ij` or `beta_ij` matched to the sites by label, and `_atom_site_U_iso_or_equiv` / `B_iso_or_equiv` for sites without an anisotropic entry (e.g. riding hydrogens, which become spheres of that size — hydrogens get no special treatment). Each site tensor goes to Cartesian coordinates (`U_cart = A·N·U_cif·N·Aᵀ`, `N = diag(|a*|,|b*|,|c*|)`), and every atom ASE generates from the site gets it rotated by the space-group operation that generated it (`R·U·Rᵀ`, `R = A·W·A⁻¹`). The principal axes are stored per vertex as `adp_rotation` (quaternion), `adp_axes` (RMS displacements, Å) and `adp_valid` (False without data or for a non-positive-definite tensor — those atoms keep their normal sphere). The atoms_and_bonds tree gets three modifier inputs: `adps` (ellipsoids ↔ normal spheres), `hydrogen_adps` (the "hydrogen ADPs" checkbox, off by default; off keeps hydrogens as the usual small spheres without rings — riding hydrogens only carry a U_iso, which at 50 % draws them larger than the atoms they sit on) and `adp_scale` (semi-axis = `adp_scale`·RMS; set from `adp_probability` at import, `√χ²₃(p)` = 1.538 for 50 %). The three principal-axis rings leave that tree as **curves**, pass the outline unshelled (the outline only offsets the mesh component), and are swept into tubes by a last `adp_rings` modifier (`ring_radius`, `profile_resolution`, `ring_material`) with the `'adp_rings'` material — a black Emission shader.
...
overlap the other way round, a weak band of one in front of a strong band of another, and no shader
